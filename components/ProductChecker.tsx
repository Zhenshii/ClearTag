         ...
         <div className="bg-white rounded-2xl shadow-xl border border-earth-200 overflow-hidden animate-slide-up">
            <div className="p-6 space-y-6">
               ...
               <div className="grid grid-cols-1 gap-6">
                  {/* Composition Section */}
                  <div className="bg-earth-50/50 p-5 rounded-2xl border border-earth-100">
                     <h4 className="font-bold text-forest-900 text-[10px] uppercase tracking-widest mb-4 opacity-60">Composition</h4>
                     <div className="space-y-3">
                        {result.composition.map((comp, idx) => (
                           <div key={idx} className="flex items-center justify-between group">
                              <div className="flex items-center gap-3">
                                 <div className={`w-2.5 h-2.5 rounded-full shadow-sm ${
                                    comp.color === 'green' ? 'bg-green-500' :
                                    comp.color === 'yellow' ? 'bg-yellow-500' :
                                    'bg-red-500'
                                 }`}></div>
                                 <span className="text-sm font-bold text-earth-800">{comp.material}</span>
                              </div>
                              <span className="text-sm font-mono font-bold text-forest-700">{comp.percentage}%</span>
                           </div>
                        ))}
                     </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {/* Negatives */}
                    {result.environmentalImpact.negatives.length > 0 && (
                      <div className="space-y-2">
                        <span className="text-[10px] font-bold text-red-600 uppercase tracking-tighter ml-1">Negatives</span>
                        <div className="flex flex-wrap gap-2">
                          {result.environmentalImpact.negatives.map((point, idx) => (
